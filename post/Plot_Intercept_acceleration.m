function Plot_Intercept_acceleration(setup, problem, c)
    ...
    switch setup.modelOptions.optimize
        ...
        case 'def'            
            T_x = problem.StateDotValues(find(ismember(problem.StateDotNames,'u_dot'),1),1:step:end);
            T_y = problem.StateDotValues(find(ismember(problem.StateDotNames,'v_dot'),1),1:step:end);
            T_z = - problem.StateDotValues(find(ismember(problem.StateDotNames,'w_dot'),1),1:step:end);
    end    

    figname = 'Intercept + Acceleration';
    ...
    if strcmp(setup.modelOptions.optimize,'def')
        ...
        p2a = quiver3(pDOO(1,1:step:end),pDOO(2,1:step:end),pDOO(3,1:step:end),T_x,T_y,T_z,'g','Linewidth',1.5);    % 3D defender trajectory with acceleration
    else
    ...
    legend([p1 p2 p2a p3 p4(1)],'Invader', 'Defender',...
        'Defender Acceleration', 'Target', 'LOS');
